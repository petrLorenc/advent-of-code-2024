...
def is_in_loop(board, actual_y_idx, actual_x_idx, obstacle_x, obstacle_y, directions, actual_direction):
    ...
    while True:
        ...
        if board[actual_y_idx][actual_x_idx] != "#" and board[actual_y_idx][actual_x_idx] != "-1":
...
cnt = 0
print(is_in_loop(copy.deepcopy(input_data), actual_y_idx, actual_x_idx, 5, 6, directions, actual_direction))
...
for x in range(x_len):
    for y in range(y_len):
        ...
        if actual_y_idx == y and actual_x_idx == x:
            continue
        if is_in_loop(copy.deepcopy(input_data), actual_y_idx, actual_x_idx, x, y, directions, actual_direction):
            cnt += 1
            print("Loop found")
print(cnt)
