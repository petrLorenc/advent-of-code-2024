...
def find_shortest_path_from_to(start_x, start_y, end_x, end_y, to_avoid="#"):
    ...
    while queue:
        ...
        for dx, dy in [(0, 1), (1, 0), (0, -1), (-1, 0)]:
            ...
            if 0 <= new_x < x_len and 0 <= new_y < y_len and board[new_y][new_x] != to_avoid:
...
def get_shortcuts(board, cost, paths, end_y, end_x):
    ...
    for idx, possible_path in enumerate(paths):
        ...
        for point in paths[idx:]:
            ...
            if c <= 20:
                new_cost = cost[start_y][start_x] + (cost[end_y][end_x] - cost[possible_y][possible_x]) + c
                shorcuts_cost.setdefault(c, []).append((new_cost, cost[end_y][end_x] - new_cost, start_y, start_x, possible_y, possible_x))
                if cost[end_y][end_x] - new_cost >= to_save: # atleast 100 picoseconds saved
                    shorcuts_cost_saving[cost[end_y][end_x] - new_cost] = shorcuts_cost_saving.get(cost[end_y][end_x] - new_cost, 0) + 1
