...
def get_fence_area(input_data, y, x, visited, visited_now):
    ...
    if (
        0 <= y - 1
        and input_data[y - 1][x] == initial_letter
        and visited_now[y - 1][x] == 0
    ):
    ...
    if (
        0 <= x - 1
        and input_data[y][x - 1] == initial_letter
        and visited_now[y][x - 1] == 0
    ):
...
for y in range(y_len):
    for x in range(x_len):
        if visited_now[y][x] == 0:
            new_area, new_fence = get_fence_area(
                input_data, y, x, visited_map, visited_now
            )
