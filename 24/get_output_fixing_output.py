...
def test_if_addition_work(values):
    ...
    return int(bin_number_z[::-1], 2) == int(bin_number_x[::-1], 2) + int(bin_number_y[::-1], 2)
...
for i in range(max_z_numbers-1):
    ...
    if new_carry is not None and new_carry.startswith("z") and new_carry != f"z{max_z_numbers-1:02d}":
